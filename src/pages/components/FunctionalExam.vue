<template>
  <div class="functional-exam">
    <Teleport v-if="!props.isDualScreen" defer to="#functional-btn-group">
        ...
        <div :class="['func-exam-dual', { 'vertical-mode': props.isDualScreen }]">
          ...
          <div class="func-exam-col">
              ...
              <div class="func-table-row">
                ...
                <div class="func-table-col result">
                  <span class="input-label">模糊点</span>
                  <a-input v-if="isEditMode" v-model:value="form.fusional_convergence_distance_blur" :readonly="!isEditMode" :disabled="false" style="width: 72px; margin-right: 8px;" placeholder="模糊点" />
                  <span v-else style="width: 72px; margin-right: 8px; display: inline-block;">{{ form.fusional_convergence_distance_blur }}</span>
                  <span class="input-label">破裂点</span>
                  <a-input v-if="isEditMode" v-model:value="form.fusional_convergence_distance_break" :readonly="!isEditMode" :disabled="false" style="width: 72px; margin-right: 8px;" placeholder="破裂点" />
                  <span v-else style="width: 72px; margin-right: 8px; display: inline-block;">{{ form.fusional_convergence_distance_break }}</span>
                  <span class="input-label">恢复点</span>
                  <a-input v-if="isEditMode" v-model:value="form.fusional_convergence_distance_recovery" :readonly="!isEditMode" :disabled="false" style="width: 72px; margin-right: 8px;" placeholder="恢复点" />
          ...
          <div class="func-exam-col">
            <div class="func-exam-col-bg">
              ...
              <div class="func-table-row">
                ...
                <div class="func-table-col result">
                  <span class="input-label">模糊点</span>
                  <a-input v-if="isEditMode" v-model:value="form.fusional_convergence_near_blur" :readonly="!isEditMode" :disabled="false" style="width: 72px; margin-right: 8px;" placeholder="模糊点" />
                  <span v-else style="width: 72px; margin-right: 8px; display: inline-block;">{{ form.fusional_convergence_near_blur }}</span>
                  <span class="input-label">破裂点</span>
                  <a-input v-if="isEditMode" v-model:value="form.fusional_convergence_near_break" :readonly="!isEditMode" :disabled="false" style="width: 72px; margin-right: 8px;" placeholder="破裂点" />
                  <span v-else style="width: 72px; margin-right: 8px; display: inline-block;">{{ form.fusional_convergence_near_break }}</span>
                  <span class="input-label">恢复点</span>
                  <a-input v-if="isEditMode" v-model:value="form.fusional_convergence_near_recovery" :readonly="!isEditMode" :disabled="false" style="width: 72px; margin-right: 8px;" placeholder="恢复点" />
...
const logFormData = () => {
  ...
  console.log('立体视:', {
    原始值: form.value.stereopsis_testing,
    显示标签: getStereopsisLabel(form.value.stereopsis_testing)
  });
  console.groupEnd();
};
...
onMounted(() => {
  ...
  logFormData();
  // 集合BO和散开BI不做自动格式化，用户可以自由输入
  // 立体视数据已经是纯数字格式，不需要额外处理
  // 从后端接收的数据应该是纯数字，可以直接使用
});
watch(() => props.record, (val) => {
  if (!val) return;
  
  // 只在数据加载时标准化眼位数据，集合BO和散开BI不做自动格式化
  [
...
// 立体视数据标准化函数 - 修改为返回纯数字
function normalizeStereopsisValue(value) {
  ...
  if (typeof value === 'string') {
    ...
    // 处理特殊值
    if (value === 'NG') return 'NG'; // 保持原样，或者根据后端要求处理
    
    // 处理字符串数字格式（兼容旧数据，如 "800.00"）
...
function handleSave() {
  ...
  const updatedRecord = {
    ...
    stereopsis_testing: form.value.stereopsis_testing // 现在已经是纯数字格式，不需要额外处理
  };
...
async function syncFromPrevious() {
  try {
    const patientId = props.patientId || props.record.patient_id;
    ...
    // 检查记录是否有视功能检查数据的辅助函数
    const hasFunctionalData = (record) => {
      return record.worth_4_type || record.aniseikonia || record.stereopsis_testing ||
             ...
             record.fusional_disvergence_distance_blur || record.fusional_disvergence_distance_break ||
             record.fusional_disvergence_distance_recovery || record.fusional_convergence_distance_blur ||
             record.fusional_convergence_distance_break || record.fusional_convergence_distance_recovery ||
             record.fusional_disvergence_near_blur || record.fusional_disvergence_near_break ||
             record.fusional_disvergence_near_recovery || record.fusional_convergence_near_blur ||
             record.fusional_convergence_near_break || record.fusional_convergence_near_recovery ||
             record.near_point_of_convergence_distance || record.positive_relative_accommodation_blur ||
             record.positive_relative_accommodation_recovery || record.negative_relative_accommodation_blur ||
             record.negative_relative_accommodation_recovery || record.accommodative_amplitude_right ||
             record.accommodative_amplitude_left || record.accommodative_amplitude_both ||
             record.accommodation_amplitude_style || record.accommodation_sensitivity_right ||
             record.accommodation_sensitivity_left || record.accommodation_sensitivity_both ||
             record.fused_cross_cylinder_right || record.fused_cross_cylinder_left ||
             record.fused_cross_cylinder_both;
    };
    ...
    for (const record of earlierRecords) {
      if (hasFunctionalData(record)) {
        previousRecord = record;
        break;
      }
    }
    
    // 步骤2: 如果没有找到早于当前日期的，则找最早日期的有视功能检查
    if (!previousRecord) {
    ...
    if (!previousRecord) {
      message.warning('未找到可同步的历史视功能检查数据');
      return;
    }
    ...
    Modal.confirm({
      ...
      content: `是否同步 ${examDate} 的视功能检查数据？`,
      okText: '确认',
      cancelText: '取消',
      onOk() {
        return new Promise((resolve) => {
          nextTick(() => {
            // 同步远距离检查数据
            form.value.worth_4_type = previousRecord.worth_4_type || form.value.worth_4_type;
            form.value.aniseikonia = previousRecord.aniseikonia || form.value.aniseikonia;
            form.value.stereopsis_testing = previousRecord.stereopsis_testing || form.value.stereopsis_testing;
            form.value.pli_exo_distance_lateral_phoria = previousRecord.pli_exo_distance_lateral_phoria || form.value.pli_exo_distance_lateral_phoria;
            form.value.plo_eso_distance_lateral_phoria = previousRecord.plo_eso_distance_lateral_phoria || form.value.plo_eso_distance_lateral_phoria;
            form.value.fusional_disvergence_distance_blur = previousRecord.fusional_disvergence_distance_blur || form.value.fusional_disvergence_distance_blur;
            form.value.fusional_disvergence_distance_break = previousRecord.fusional_disvergence_distance_break || form.value.fusional_disvergence_distance_break;
            form.value.fusional_disvergence_distance_recovery = previousRecord.fusional_disvergence_distance_recovery || form.value.fusional_disvergence_distance_recovery;
            form.value.fusional_convergence_distance_blur = previousRecord.fusional_convergence_distance_blur || form.value.fusional_convergence_distance_blur;
            form.value.fusional_convergence_distance_break = previousRecord.fusional_convergence_distance_break || form.value.fusional_convergence_distance_break;
            form.value.fusional_convergence_distance_recovery = previousRecord.fusional_convergence_distance_recovery || form.value.fusional_convergence_distance_recovery;
            
            // 同步近距离检查数据
            form.value.pli_exo_near_lateral_phoria = previousRecord.pli_exo_near_lateral_phoria || form.value.pli_exo_near_lateral_phoria;
            form.value.plo_eso_near_lateral_phoria = previousRecord.plo_eso_near_lateral_phoria || form.value.plo_eso_near_lateral_phoria;
            form.value.fusional_disvergence_near_blur = previousRecord.fusional_disvergence_near_blur || form.value.fusional_disvergence_near_blur;
            form.value.fusional_disvergence_near_break = previousRecord.fusional_disvergence_near_break || form.value.fusional_disvergence_near_break;
            form.value.fusional_disvergence_near_recovery = previousRecord.fusional_disvergence_near_recovery || form.value.fusional_disvergence_near_recovery;
            form.value.fusional_convergence_near_blur = previousRecord.fusional_convergence_near_blur || form.value.fusional_convergence_near_blur;
            form.value.fusional_convergence_near_break = previousRecord.fusional_convergence_near_break || form.value.fusional_convergence_near_break;
            form.value.fusional_convergence_near_recovery = previousRecord.fusional_convergence_near_recovery || form.value.fusional_convergence_near_recovery;
            form.value.near_point_of_convergence_distance = previousRecord.near_point_of_convergence_distance || form.value.near_point_of_convergence_distance;
            
            // 同步调节相关数据
            form.value.positive_relative_accommodation_blur = previousRecord.positive_relative_accommodation_blur || form.value.positive_relative_accommodation_blur;
            form.value.positive_relative_accommodation_recovery = previousRecord.positive_relative_accommodation_recovery || form.value.positive_relative_accommodation_recovery;
            form.value.negative_relative_accommodation_blur = previousRecord.negative_relative_accommodation_blur || form.value.negative_relative_accommodation_blur;
            form.value.negative_relative_accommodation_recovery = previousRecord.negative_relative_accommodation_recovery || form.value.negative_relative_accommodation_recovery;
            form.value.accommodative_amplitude_right = previousRecord.accommodative_amplitude_right || form.value.accommodative_amplitude_right;
            form.value.accommodative_amplitude_left = previousRecord.accommodative_amplitude_left || form.value.accommodative_amplitude_left;
            form.value.accommodative_amplitude_both = previousRecord.accommodative_amplitude_both || form.value.accommodative_amplitude_both;
            form.value.accommodation_amplitude_style = previousRecord.accommodation_amplitude_style || form.value.accommodation_amplitude_style;
            form.value.accommodation_sensitivity_right = previousRecord.accommodation_sensitivity_right || form.value.accommodation_sensitivity_right;
            form.value.accommodation_sensitivity_left = previousRecord.accommodation_sensitivity_left || form.value.accommodation_sensitivity_left;
            form.value.accommodation_sensitivity_both = previousRecord.accommodation_sensitivity_both || form.value.accommodation_sensitivity_both;
            form.value.fused_cross_cylinder_right = previousRecord.fused_cross_cylinder_right || form.value.fused_cross_cylinder_right;
            form.value.fused_cross_cylinder_left = previousRecord.fused_cross_cylinder_left || form.value.fused_cross_cylinder_left;
            form.value.fused_cross_cylinder_both = previousRecord.fused_cross_cylinder_both || form.value.fused_cross_cylinder_both;
            
            message.success(`已同步 ${examDate} 的视功能检查数据`);
            resolve();
          });
        });
      }
    ...
    console.error('同步视功能检查失败:', error);
    message.error('同步视功能检查失败: ' + error.message);
  }
}
