                ...
                <a-tab-pane key="active" tab="活跃统计">
                  <div class="stats-section">
                    ...
                    <div class="daily-stats">
                      <h6>每日详细数据：</h6>
                      <div v-for="dayData in activeStatsData" :key="`active-${dayData.queryDate}`" class="day-item">
                        <div class="day-header">
                          <span class="day-date">{{ dayData.queryDate }}</span>
                          <span class="day-total">总计: {{ (dayData.review || 0) + (dayData.register || 0) }}</span>
                        </div>
                        <div class="day-details">
                          <span class="detail-item">复查: {{ dayData.review }}</span>
                          <span class="detail-item">新增: {{ dayData.register }}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="stats-section">
                    <div class="stats-summary">
                       ...
                       <div class="summary-item">
                         ...
                         <span class="value">{{ totalWorkStats.incomplete }}</span>
                       </div>
                       <div class="summary-item">
                         <span class="label">总需提醒数：</span>
                         <span class="value">{{ totalWorkStats.reminder }}</span>
                       </div>
                     </div>
                     ...
                     <div class="daily-stats">
                       ...
                       <div v-for="dayData in workStatsData" :key="`work-${dayData.queryDate}`" class="day-item">
                         ...
                         <div class="day-details">
                           <span class="detail-item">未完善: {{ dayData.incomplete }}</span>
                           <span class="detail-item">需提醒: {{ dayData.reminder }}</span>
                         </div>
                       </div>
                     </div>
                  </div>
...
const saveTodayActiveDataToIndexedDB = async (data) => {
  try {
    ...
    console.log('单日活跃数据已保存到 IndexedDB:', data.queryDate);
...
const fetchTodayActiveStats = async (organizations, queryDate = null) => {
  try {
    ...
    const orgDetails = {};
     ...
     // 遍历所有机构，获取每个机构的今日活跃数据
     for (const org of organizations) {
       try {
         // 获取指定日期的建档数据
         const registerResponse = await fetch('https://aiforoptometry.com/medicalrecords/search_same_day_registered', {
           method: 'POST',
           headers: {
             'Content-Type': 'application/json',
             'X-CSRFToken': csrftoken
         ...
         const todayRegisterCount = registerData.patients ? registerData.patients.length : 0;

         // 获取指定日期的复查数据
         const reviewResponse = await fetch('https://aiforoptometry.com/medicalrecords/search_same_day_recheck', {
           method: 'POST',
           headers: {
             'Content-Type': 'application/json',
             'X-CSRFToken': csrftoken
         ...
         const todayReviewCount = reviewData.patients ? reviewData.patients.length : 0;
...
const fetchMissingDatesData = async (dates) => {
  try {
    ...
    const promises = dates.map(async (date) => {
      try {
        console.log(`正在获取日期 ${date} 的数据...`);
        
        // 只获取新增和复查数据，不影响其他字段
        const activeData = await fetchActiveStatsForModal(organizations, date);
...
const updateTotalActiveStats = (dataArray) => {
  ...
  console.log('总计活跃统计数据已更新（新增 + 复查）:', totals);
  console.log('更新后的 totalActiveStats:', totalActiveStats.value);
};
...
const updateTotalWorkStats = (dataArray) => {
  ...
  dataArray.forEach((item, index) => {
    ...
    const itemTotal = itemIncomplete + itemReminder;
    
    // 工作统计只关心未完善和需提醒，总数就是这两项的和
    totals.total += itemTotal;
    totals.incomplete += itemIncomplete;
    totals.reminder += itemReminder;
    
    console.log(`工作统计项目 ${index + 1} (${item.queryDate}):`, {
      未完善: itemIncomplete,
      需提醒: itemReminder,
      小计: itemTotal
    });
  ...
  console.log('总计工作统计数据已更新（未完善 + 需提醒）:', totals);
  console.log('更新后的 totalWorkStats:', totalWorkStats.value);
};
...
const exportActiveStatsToExcel = async () => {
  try {
    ...
    // 添加每日数据到活跃统计表
    activeStatsData.value.forEach(dayData => {
    ...
    // 添加汇总行
    const totalDays = activeStatsData.value.length;
    const totalReviewSum = activeStatsData.value.reduce((sum, dayData) => sum + (dayData.review || 0), 0);
    const totalRegisterSum = activeStatsData.value.reduce((sum, dayData) => sum + (dayData.register || 0), 0);
    ...
    for (const org of selectedOrgs) {
      ...
      orgSheetData.push([
        `天数(${activeStatsData.value.length}天)`,
        orgTotalReview,
        orgTotalRegister,
        orgTotalSum
      ]);
      ...
      console.log(`机构 ${org.name} 活跃统计汇总数据:`, {
        天数: activeStatsData.value.length,
        复查总量: orgTotalReview,
    ...
    const fileName = `活跃统计数据表_${startDate}至${endDate}.xlsx`;
  ...
  try {
    ...
    for (const org of organizations) {
      try {
        ...
        // 累加数据
        totalRegister += todayRegisterCount;
        totalReview += todayReviewCount;
        
        // 保存每个机构的详细数据（只包含新增和复查）
        orgDetails[org.id] = {
          name: org.name,
          today_review: todayReviewCount,
          today_register: todayRegisterCount,
        ...
        console.log(`机构 ${org.name} 活跃统计数据:`, {
          新增: todayRegisterCount,
          复查: todayReviewCount,
          总计: todayReviewCount + todayRegisterCount
        });
        ...
        console.error(`获取机构 ${org.name} 活跃统计数据失败:`, error);
        // 如果获取失败，使用0作为默认值
        orgDetails[org.id] = {
          name: org.name,
          today_review: 0,
...
const renderTodayActiveChart = () => {
  ...
  try {
    ...
    // 先清空图表，再设置新数据
    isolatedTodayActiveChartInstance.clear();
    isolatedTodayActiveChartInstance.setOption(option);
    
    // 添加图例点击事件监听
    isolatedTodayActiveChartInstance.on('legendselectchanged', function(params) {
      console.log('今日活跃数据图表图例变化:', params);
      // 可以在这里添加自定义逻辑，比如更新其他相关显示
    });
...
onMounted(async () => {
  try {
    ...
    try {
      ...
      console.warn('IndexedDB 初始化失败，将使用 localStorage 作为备选:', error);
    }
    
    // 先尝试从缓存加载统计数据
    const cachedStats = loadCachedOrganizationStats();
    if (cachedStats) {
      console.log('页面加载时使用缓存的统计数据');
      stats.value.totalPatients = cachedStats.totalPatients;
      stats.value.totalExaminations = cachedStats.totalExaminations;
    } else {
      console.log('没有缓存数据，需要请求新数据');
    }
    
    // 先尝试从缓存加载单日活跃数据
    const cachedTodayActive = loadCachedTodayActiveData();
    if (cachedTodayActive) {
      console.log('页面加载时使用缓存的单日活跃数据');
      ...
      console.log('没有单日活跃缓存数据，需要请求新数据');
    }
