<template>
  <a-card :body-style="{ padding: '0' }" class="print-preview-card">
    ...
    <div class="preview-container">
              <div class="a4-preview-box" data-print-target="review-report">
...
.a4-preview-box {
  /* 强制A4尺寸：210mm x 297mm，转换为像素约为794px x 1123px */
  width: 794px;
  height: 1123px;
  /* 移除响应式限制，确保始终是固定A4尺寸 */
  background: #fff;
...
@media print {
  ...
  * {
  ...
  .print-preview-area {
    ...
    height: calc(297mm - 30mm) !important; /* A4高度减去上下边距 */
...
@media screen and (max-width: 768px) {
  ...
  /* 保持固定字体尺寸，不进行响应式调整 */
  .report-table {
    font-size: 15px; /* 保持固定字体大小 */
  }
  .report-table th,
  .report-table td {
    padding: 8px 0; /* 保持固定内边距 */
  }
...
/* 超小屏设备响应式调整（手机等） - 保持A4固定尺寸，字体也保持固定尺寸 */
@media screen and (max-width: 480px) {
  ...
  .print-preview-area {
    padding: 24px; /* 保持固定内边距 */
  }
  
  .report-title {
    font-size: 60px; /* 从 30px 改为 60px，放大2倍 */
    letter-spacing: 6px; /* 从 3px 改为 6px，保持比例 */
  }
  
  .patient-info-row {
    font-size: 24px; /* 保持固定字体大小 */
  }
}
...
.logo-fixed {
  position: absolute;
  top: 10px;
  right: 40px;  /* 从 left: 32px 改为 right: 40px，移到右上角 */
...
.orgname-fixed {
  position: absolute;
  top: 14px;
  left: 32px;  /* 从 right: 40px 改为 left: 32px，移到左上角 */
  font-size: 24px;
  font-weight: 700;
  color: #1976d2;
  letter-spacing: 2px;
...
function renderAxialChart() {
  ...
  // 简化图表配置，使用固定值，让CSS transform自动处理缩放
  const fontSize = 12;
  const gridConfig = {
    left: 60,
    right: 30,
  ...
  console.log('【图表配置】使用简化配置，让CSS自动处理缩放');
  ...
  setTimeout(() => {
      try {
        // 检查图表实例是否还存在且未被销毁
        if (chartInstance && !chartInstance.isDisposed()) {
          const base64 = chartInstance.getDataURL({ pixelRatio: 2, backgroundColor: '#fff' });
          if (base64) {
...
// 确保A4预览框保持固定尺寸，不进行动态缩放
function ensureA4FixedSize() {
  const box = document.querySelector('.a4-preview-box');
  if (!box) return;
  
  // 强制重置为固定A4尺寸，移除任何动态缩放
  ...
  console.log('【A4固定尺寸】确保预览框保持标准A4尺寸: 794px x 1123px');
}
...
onBeforeUnmount(() => {
  window.removeEventListener('resize', ensureA4FixedSize);
  // 清理图表实例
  if (chartInstance) {
    chartInstance.dispose();
    chartInstance = null;
